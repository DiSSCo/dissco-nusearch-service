...
// https://github.com/gbif/checklistbank/blob/master/checklistbank-common/src/main/java/org/gbif/checklistbank/authorship/AuthorComparator.java
...
import eu.dissco.nusearch.utils.YearComparator;
...
/**
 * Utility to compare scientific name authorships, i.e. the recombination and basionym author and
 * publishing year. Author strings are normalized to ASCII and then compared. As authors are often
 * abbreviated in all kind of ways a shared common substring is accepted as a positive equality. If
 * any of the names given has an empty author & year the results will always be Equality.UNKNOWN.
 * <p>
 * The class exposes two kind of compare methods. A strict one always requiring both year and author
 * to match and a more lax default comparison that only looks at years when the authors differ (as
 * it is quite hard to compare authors)
 */
public class AuthorComparator {

  private static final Logger LOG = LoggerFactory.getLogger(AuthorComparator.class);

  private static final Pattern AND = Pattern.compile("( et | and |&|&amp;)",
  ...
  private static final Pattern TRANSLITERATIONS = Pattern.compile("([auo])e",
      Pattern.CASE_INSENSITIVE);
  ...
  public static AuthorComparator createWithAuthormap() {
    try {
      AuthorComparator ac = new AuthorComparator(
          FileUtils.streamToMap(
              Resources.asByteSource(AuthorComparator.class.getResource(AUTHOR_MAP_FILENAME))
                  .openStream(),
  ...
  /**
   * Compares the author and year of two names by first evaluating equivalence of the authors. Only
   * if they appear to differ also a year comparison is done which can still yield an overall EQUAL
   * in case years match.
   */
  public Equality compare(@Nullable String authors1, @Nullable String year1,
      @Nullable String authors2, @Nullable String year2) {
    // compare recombination authors first
    Equality result = compareAuthorteam(authors1, authors2, minCommonSubstring,
        MIN_AUTHOR_LENGTH_WITHOUT_LOOKUP);
  ...
  /**
   * Does a comparison of recombination and basionym authorship using the author compare method once
   * for the recombination authorship and once for the basionym.
   */
  public Equality compare(ParsedName n1, ParsedName n2) {
    ...
    Equality original = compare(n1.getBracketAuthorship(), n1.getBracketYear(),
        n2.getBracketAuthorship(), n2.getBracketYear());
    if (original == Equality.UNKNOWN) {
      ...
      if (Strings.isNullOrEmpty(n1.getAuthorship()) && Strings.isNullOrEmpty(n1.getYear())) {
        across = compare(n1.getBracketAuthorship(), n1.getBracketYear(), n2.getAuthorship(),
            n2.getYear());
      } else if (Strings.isNullOrEmpty(n1.getBracketAuthorship()) && Strings.isNullOrEmpty(
          n1.getBracketYear())) {
        across = compare(n1.getAuthorship(), n1.getYear(), n2.getBracketAuthorship(),
            n2.getBracketYear());
      }
  ...
  /**
   * Compares two sets of author & year for equality. This is more strict than the normal compare
   * method and requires both authors and year to match. Author matching is still done fuzzily
   *
   * @return true if both sets match
   */
  public boolean compareStrict(String author1, @Nullable String year1, String author2,
      @Nullable String year2) {
  ...
  private List<String> splitAndLookup(String normalizedAuthorTeam,
      int minAuthorLengthWithoutLookup) {
    List<String> authors = Lists.newArrayList();
  ...
  /**
   * Does an author comparison, normalizing the strings and try 3 comparisons: 1) checks regular
   * string equality 2) checks for equality of the longest common substring 3) do an author lookup
   * and then check for common substring
   */
  private Equality compareAuthorteam(@Nullable String a1, @Nullable String a2,
      int minCommonSubstring, int maxAuthorLengthWithoutLookup) {
   ...
   */
  private Equality compareNormalizedAuthorteam(final List<String> authorTeam1,
      final List<String> authorTeam2, final int minCommonStart) {
  ...
  static class Author {
    ...
    private Equality compare(final Author other, final int minCommonStart) {
      if (fullname.equals(other.fullname)) {
        ...
        } else if (!firstInitialsDiffer(other) && (
            surname.equals(common) && (other.surname.startsWith(common))
        ...
        } else if (lengthWithoutWhitespace(StringUtils.getCommonPrefix(fullname, other.fullname))
            > minCommonStart) {
